...
async function generateText(provider, model, messages, signal) {
  let output = '';
...
export async function generatePatchProposal({
  ...
  const critic = await generateText(provider, model, [{ role: 'system', content: reviewPrompt }, { role: 'user', content: 'Review this patch now.' }], signal);
