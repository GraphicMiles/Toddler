...
export default function MobileDashboard() {
  ...
  const download = async model => {
    setDownloading(model.id); setDownloadProgress(0); setDownloadError('')
    try {
      if (!navigator.onLine) throw new Error('You are offline. Connect to the internet to download this model.')
      if (!model.downloadUrl) throw new Error('This model is not published for download yet.')
      if (model.downloadUrl) {
        ...
        const blob = new Blob(reader ? chunks : [await response.arrayBuffer()])
        const expectedBytes = Number(model.sizeBytes || 0)
        if (expectedBytes && blob.size < expectedBytes * 0.95) throw new Error('Downloaded model is incomplete. Please retry.')
        await localforage.setItem(`toddler-model-${model.id}`, blob)
      }
      saveModels([...new Set([...downloaded, model.id])]); setTab('train')
    } catch (error) { setDownloadError(error.message) } finally { setDownloading(null); setDownloadProgress(0) }
  }
