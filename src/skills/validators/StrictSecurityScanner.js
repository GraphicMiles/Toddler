...
export class StrictSecurityScanner extends SkillValidator {
  getDescription() {
    return 'Enterprise-grade security scanner. Rejects critical vulnerabilities.';
  }
  ...
  validate(skill, files = {}) {
    ...
    for (const [path, raw] of Object.entries(files)) {
      ...
      if (new TextEncoder().encode(content).byteLength > MAX_FILE_BYTES) {
        findings.push(createFinding('FILE_TOO_LARGE', 'critical', 'Skill file exceeds the scanner limit.', path));
        continue;
      }

      if (SCRIPT_EXTENSIONS.test(path)) {
        findings.push(createFinding('ANDROID_SCRIPT_DISABLED', 'info', 'Android stores skill scripts as inert resources.', path));
      }

      for (const pattern of PATTERNS) {
        if (!pattern.expression.test(content)) continue;
        const severity = pattern.id === 'NETWORK' && declaredNetwork ? 'info' : pattern.severity;
        findings.push(createFinding(pattern.id, severity, pattern.message, path));
      }
    }
