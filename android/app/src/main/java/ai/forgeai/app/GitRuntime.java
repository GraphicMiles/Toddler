...
public class GitRuntime extends Plugin {
    ...
    private File safeRepository(String path) throws Exception {
        ...
        if (!repository.toPath().startsWith(root.toPath()) || !new File(repository, ".git").isDirectory()) throw new IllegalArgumentException("Git repository must be a ForgeAI app-private clone.");
        return repository;
    }
    ...
    public void commit(PluginCall call) {
        executor.execute(() -> {
            try (Git git = Git.open(safeRepository(call.getString("path", "")))) {
                ...
                RevCommit commit = git.commit().setMessage(message).setAuthor(call.getString("authorName", "ForgeAI User"), call.getString("authorEmail", "forgeai@localhost")).call();
