...
export default function MobileDashboard() {
  ...
  const recommended = models.filter(model => model.ram <= availableRam)
  ...
  return <div className="mobile-app">
    ...
    <main className="mobile-main">
      ...
      {tab === 'zoo' && <>
        ...
        <div className="model-list">{models.map(model => { const isDownloaded = downloaded.includes(model.id); const fits = model.ram <= availableRam; return <article className={`model-card ${!fits ? 'disabled' : ''}`} key={model.id}>
      ...
      {tab === 'train' && <div className="empty-panel"><Play size={28}/><h2>Your downloaded models</h2>{downloaded.length ? <div className="downloaded-list">{models.filter(m=>downloaded.includes(m.id)).map(m=><div className="downloaded-row" key={m.id}><div><b>{m.name}</b><span>{m.size} MB · ready to train</span></div><button className="small-button" onClick={() => alert('Dataset picker coming next')}>Upload dataset <ChevronRight size={14}/></button></div>)}</div> : <><p>Download a model from the Model Zoo to start training locally.</p><button className="primary-button" onClick={() => setTab('zoo')}>Browse Model Zoo</button></>}</div>}
