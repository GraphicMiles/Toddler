...
export default function MobileDashboard() {
  ...
  const recommended = catalog.filter(model => modelRam(model) <= availableRam)
  ...
  return <div className="mobile-app">
    ...
    <main className="mobile-main">
      ...
      {tab === 'zoo' && <><div className="section-heading"><div><p className="mobile-kicker">MODEL ZOO</p><h2>Recommended for you</h2></div><span className="model-count">{recommended.length} compatible</span></div><div className="model-filters">{['All','Text','Vision','Embeddings','Recommended','Downloaded'].map(item => <button key={item} className={category === item ? 'active' : ''} onClick={() => setCategory(item)}>{item}</button>)}</div><div className="model-list">{visibleModels.map(model => { const isDownloaded = downloaded.includes(model.id); const fits = modelRam(model) <= availableRam; return <article className={`model-card ${!fits ? 'disabled' : ''}`} key={model.id}><div className="model-icon" style={{ color: model.color }}><Zap size={19}/></div><div className="model-info"><div className="model-title"><h3>{model.name}</h3>{isDownloaded && <CheckCircle2 size={16} className="success"/>}</div><p>{model.type}</p><small>{model.description}</small><div className="model-meta"><span>{modelSize(model)} MB</span><span>{model.params || `${(model.parameterCount || 0) / 1000000}M`} params</span><span>~{formatRam(modelRam(model))} RAM</span></div></div><button className="model-action" disabled={!fits || downloading === model.id || isDownloaded} onClick={() => download(model)}>{isDownloaded ? 'Downloaded' : downloading === model.id ? `Downloading ${downloadProgress}%…` : fits ? <><Download size={15}/> Train model</> : 'Not compatible'}</button></article>})}</div></>}
